...
session-projection seam 通过 [dsh-session-projection](../../packages/session/session-projection)（`ctx.sessionProjections`）向调用方提供从日志推导的完整会话状态。注册表订阅已提交的 `session/event`，并经纯 projection 单元折叠事件；领域包无需维护额外订阅。[包 README](../../packages/session/session-projection/README.zh.md)说明驱动、缓存与变更通知约定。
