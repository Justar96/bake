...
Match order is exact routes, longest matching prefix, then the registered fallback. A second fallback registration throws.
...
`WebServer` (`ctx.webServer`) starts listening on activation; a listen failure rejects initialization. `register(route)` adds a named route and returns its disposer, while a duplicate `(kind, path)` throws. `port` returns the listening port. See the [package README](../../packages/host/webserver/README.md) for operational detail.
