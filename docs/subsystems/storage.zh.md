...
存储子系统通过 [dsh-storage](../../packages/storage/storage) 与 JSON 后端 [dsh-storage-json](../../packages/storage/storage-json) 持久保存非会话日志数据；[dsh-storage-domain](../../packages/storage/storage-domain) 提供类型化领域形式。会话日志使用独立的[持久化能力](persistence.zh.md)。
