...
Web 访问通过 [dsh-web](../../packages/web/web) 提供搜索与获取能力。搜索提供方是 [Exa](../../packages/web/web-search-exa) 和 [DeepSeek](../../packages/web/web-search-deepseek)，页面获取由 [HTTP 提供方](../../packages/web/web-fetch-http) 处理；[dsh-tool-web](../../packages/web/tool-web) 暴露 `web_search` 与 `web_fetch`。
