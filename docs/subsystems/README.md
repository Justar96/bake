...
> Type declarations and their JSDoc on these pages are source-equivalent and drift-checked by `scripts/verify-type-equiv.ts` (see [development.md](../development.md#documenting-types-verbatim)). Ordinary blocks preserve complete declarations; `public-api` blocks preserve body-stripped public class declarations. Cordis services and events use each page's generated **Cordis API** section.
