...
The glyph takes the rail's first column, level with an action's marker. The draft and status line start two columns in. The goal stays at the header's right edge, beside the turn's processing state or outcome. `headerRoom` gives the turn its label first: the goal's details are cut from their end while a few cells of them fit, then left out so `● Goal active` stands alone, and below that the goal is dropped whole rather than left as a fragment. With neither a turn nor a goal the header is a blank row, keeping the input in place. The goal's states are `● Goal active` in orange with its round count while the goal may continue, `○ Goal on hold` or `○ Goal paused` in yellow while it waits on `/goal resume`, `✗ Goal blocked` in red with its reason, and `✓ Goal complete` in green. The objective follows the state when room permits. A dim `Ctrl+O` leads the goal, naming the key that opens its full view; it is the only standing hint on the row, and a tight row gives it up after the goal's details and before its label. The `/goal` command shows the complete goal and its actions. The rules' glyphs are the style resolved from the terminal ([§4](#the-frame-is-chosen-from-the-terminal-not-assumed)), because a full-width run of East Asian Ambiguous glyphs is the one place such a character accumulates error across a row.
...
| action | the tool's icon (`$` a command, `≡` a read, `✎` an edit, `→` a message to a subagent; `●` for any other), then `Tool(argument)` | head at 2 | marker orange and blinking on and off while running, green when done, red on failure; tool name bold |
