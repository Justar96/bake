 ...
 * can present the screen between those writes, with the composer, the status
 * line, and the turn header gone. A streaming answer prints a row each time a
 * line finishes, so the input's frame blinks out and back once per line.
 *
 * Every write Ink makes in one synchronous render is held and written
 * together, in order, so a frame reaches the terminal as one write. Inside
 * that write, an erase of the dynamic region becomes a move to its top. Each
 * following row is cleared as it is drawn. Whatever remains of the old frame
 ...
 * Ink clears the screen to replay history. The composer stays at the bottom
 * from the first frame instead of following history down to it. The interface
 ...
 * the terminal. The rows after such a sequence are not the rows the erase
 * would have cleared, so an overwrite in progress closes before it.
 */
const MOVES = /\u001B\[\d*[ABEFHfJ]|\u001B\[\d*;\d*[Hf]|\u001Bc|\u001B\[\?1049[hl]/

/**
 * Join one render's writes, drawing each erased region over instead of erasing it first.
 *
 * Pure, so the rewrite can be checked against byte sequences without a
 * terminal. Writes this function does not recognize pass through unchanged.
 *
...
export function overwrite(chunks: readonly string[]): string {
  let out = ''
  // Rows are being drawn over an erased region. That region still holds the
  // previous frame below the cursor.
  let open = false
  const close = (): void => {
  ...
  for (const chunk of chunks) {
    if (ERASE_REGION.test(chunk)) {
      ...
      // Clear the first row as it is drawn. Erasing it after its text would
      // also erase a full-width row's last cell, where the cursor rests until
      // the next character wraps it.
 ...
 * which happens once after a narrowing. Replayed from the top, history shorter
 * than the screen leaves the frame partway down. Replayed from the bottom row,
 * history scrolls up to meet the frame there.
 *
 ...
 *
 * Ink's incremental renderer rewrites a changed row followed by a newline, but
 * steps over an unchanged row with Cursor Next Line. That sequence stops at
 * the bottom row instead of scrolling. The frame rests on the bottom row, so
 * when it grows and an unchanged row — often a blank from the composer
 * padding — falls past the old bottom, that row is never added. The frame is
 * then one row short, Ink's line count no longer matches the screen, and the
 * next erase takes a row of history with it. Above the bottom row the two
 * moves are the same.
 *
...
export function frameOutput(out: NodeJS.WriteStream, err: NodeJS.WriteStream, styles = true): FrameOutput {
  ...
  const flush = (): void => {
    ...
    for (let start = 0; start < held.length;) {
      ...
      if (end === start) {
        // Binary, not text. Write the bytes as they are, in their place.
