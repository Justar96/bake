...
The [whole-process diagnostic](packages/app/performance/README.md) uses the shared Bun production build and native PTY to measure Node profile readiness, input echoes, streaming, retained heap, and peak RSS with fixed synthetic histories. It separates first usable input from complete replay and covers fresh, mixed, tool-heavy, and large-answer histories. The measurement contract and raw results remain with that owner.
...
The status-line subagent count derives identities from `subagents.listChildren` and activity from live Agent status and scoped subagent run events. Down from an empty composer selects it; Enter and Ctrl+G open the subagent sheet, whose Enter runs `/agents <id>` for the child under its pointer, and `/agents` alone opens a filterable picker. A selected child is observed through `sessionQuery.observeSession`, with a sequence-fenced buffered tail and live assistant frames. Inspection owns only observers: it neither resumes nor disposes the child. Escape returns to the mounted parent composer, and parent interactions take focus. Remote runs without local transcripts and unreadable catalog entries report their unavailable state.
