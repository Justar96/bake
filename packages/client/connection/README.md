...
The Connection service carries typed requests and events between a Client and Host. API Gateway owns its Remote stream, and compositions can supply a carrier through `connection.rpc.open`. Each installer invocation creates one Context-owned service; unclaimed exact routes return 404.
