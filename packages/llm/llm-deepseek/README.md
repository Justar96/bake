...
When the composition includes the command registry, `/usage` shows the DeepSeek account's remaining balance by currency: total, unexpired granted credit, topped-up credit, and whether API calls are available. It uses the current provider credential and requests `GET /user/balance` on the configured API origin; a gateway must implement that endpoint. The command does not call a model or report historical token usage. Escape cancels the request. Authentication, HTTP, timeout, and invalid-response failures appear as command errors; the balance result is recorded with the command in the Session log.
