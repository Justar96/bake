...
组合包含命令注册表时，`/usage` 按币种显示 DeepSeek 账户的剩余额度：总额、未过期的赠送额度、充值额度，以及 API 调用是否可用。命令使用当前提供方凭据，在已配置的 API 来源请求 `GET /user/balance`；网关必须实现该端点。命令不会调用模型，也不报告历史 token 用量。按 Escape 可取消请求。身份验证、HTTP、超时及无效响应会显示为命令错误；余额结果随命令记录在会话日志中。
