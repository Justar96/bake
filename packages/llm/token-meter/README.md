...
`contextBreakdown` retains plain-JSON `{ seq, heuristicTokens, system }` entries in surface order and reuses the measurement plan/commit fold. Its state and surface transitions are O(current retained surface), not O(1) and not O(total historical log); replaced entries and message bodies are not retained. State version 5 invalidates checkpoints priced with offload metadata. `contextPressure` retains scalar shadow prices alongside the request and sampled routes; replacements without adjacent claims contribute zero delta. Its version 6 state is rebuilt from the log when an older projection checkpoint is encountered. The usage fold retains one last-sample slot because legal logs never report usage for an earlier step after a later step reports usage.
