...
`/permission` 命令把当前配置的预设名称作为参数选项提供。提供器读取实时预设列表，包括可用时的 Auto；提交的名称仍由命令校验。
...
切换到 Auto 时，服务先同步执行其准入检查；每次预设切换随后只改变实际值不同的旋钮，再次选择当前已生效的预设不会产生任何变化。当前值解析顺序为：仍匹配的最近一次记录选择，其次是配置表中的第一个匹配项，否则为 `custom`。命令显示 `/permission [preset]`：不带参数调用时报告当前预设与所有可用条目，带预设参数时切换过去。未知名称会返回 `Usage: /permission [preset]`，随后列出未知名称与可用条目。
