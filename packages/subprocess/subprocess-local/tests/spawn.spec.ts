...
describe('output truncation and spill', () => {
  ...
  it('settles a real stream when its spill directory is gone', async () => {
    const removedDir = mkdtempSync(join(tmpdir(), 'dsh-subprocess-removed-stream-'))
    rmSync(removedDir, { recursive: true, force: true })
    const failures: { error: unknown; label: string }[] = []
    const result = await finish(spawnSubprocess(
      spec('printf "%.0sx" $(seq 1 500)', { stdoutMaxBytes: 10 }),
      { spillDir: removedDir, onSpillFailure: (error, label) => { failures.push({ error, label }) } },
    ))
    expect(result.exitCode).toBe(0)
    expect(result.signal).toBeNull()
    expect(result.stdout).toEqual({ text: 'x'.repeat(10), truncated: true })
    expect(failures).toHaveLength(1)
    expect(failures[0]!.label).toBe('stdout')
