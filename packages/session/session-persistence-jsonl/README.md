...
During discovery, `list()` omits a session whose compressed header frame is corrupt and continues listing healthy sessions without changing the damaged file. A targeted `stat(id)` or `open(id)` still reports that corruption; header I/O failures and cancellation still propagate.
