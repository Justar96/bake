...
export function render(entries: CatalogEntry[]): string {
  ...
  const lines: string[] = [
    ...
    '     Run `bun scripts/gen-config-catalog.ts` to regenerate. -->',
    '',
    ...
    '',
    'This file is GENERATED from source (`scripts/gen-config-catalog.ts`) and checked with `bun scripts/gen-config-catalog.ts --check` — do not edit it by hand. Declaration blocks use a `ts config-catalog` fence (skipped by doc-typecheck, since a lone declaration referencing imports is not standalone-compilable). The generator also cross-checks the runtime schemastery schema against the pasted declaration — every schema-validated key, nested keys included, must be locatable on the declared config type — so the paste cannot hide a loader-accepted field.',
    '',
...
function main(): void {
  ...
  if (process.argv.includes('--check')) {
    ...
    console.error(`gen-config-catalog: ${OUT} is stale. Run \`bun scripts/gen-config-catalog.ts\` and commit ${OUT}.`)
    process.exit(1)
  }
