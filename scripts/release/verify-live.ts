 ...
 *
 * Waits for `latest.json` to name `version`, then verifies it the way a
 * client does: a signature from a committed key alone, whatever
 * `BAKE_RELEASE_PUBLIC_KEY` says, and every listed archive's size, fetched
 * whole and hashed. A deployment that reports success is not a release until
 * this passes.
 */
...
export async function verifyLive(options: LiveOptions): Promise<ReleaseManifest> {
  ...
  for (const path of options.base.startsWith('https://github.com/') ? [] : ['health', 'install.sh', 'install.ps1']) {
    const response = await fetcher(`${options.base}/${path}`, { cache: 'no-store' })
    if (!response.ok) throw new Error(`${options.base}/${path} answered ${response.status}`)
  }
  return manifest
}
